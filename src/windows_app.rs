...
impl WindowsApp {
        ...
        card(ui, |ui| {
            ...
            settings_row(
                ...
                self.recorded_key
                    ...
                    .unwrap_or_else(|| "Record a key or combination".into()),
                |ui| {
                    if ui
                        .add_sized(
                            [132.0, 28.0],
            ...
            let position_text = self
                .engine
                .position()
                .map(|position| format!("Target: ({}, {})", position.x, position.y))
                .unwrap_or_else(|| "Uses the current pointer position".into());
            ...
            row_separator(ui);
            settings_row(ui, "Interval", "Delay between actions", |ui| {
                ui.horizontal(|ui| {
                    ui.add(
    ...
    fn hotkey_subtitle(&self) -> String {
        if self.hotkey_thread.available() {
            "Works while the app is in the background".into()
        } else {
            format!(
...
mod tests {
    ...
    #[test]
    fn compact_settings_row_stays_short() {
        let context = eframe::egui::Context::default();
        configure_style(&context);
        let input = eframe::egui::RawInput {
            screen_rect: Some(eframe::egui::Rect::from_min_size(
                eframe::egui::Pos2::ZERO,
                eframe::egui::vec2(450.0, 100.0),
            )),
            ..Default::default()
        };
